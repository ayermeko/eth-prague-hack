...
```
Clients that support remote MCP (Codex, Cursor, VS Code, Copilot) use OAuth. User is prompted to sign in on first use.
...
- Generate ready-to-paste MCP configs for specific clients (Codex, Cursor, VS Code, Copilot, Windsurf, Codex Desktop, and generic stdio)
